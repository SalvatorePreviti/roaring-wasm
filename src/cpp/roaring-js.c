...
uint32_t get_sizeof_roaring_bitmap_t() { return sizeof(roaring_bitmap_t); }
...
int roaring_bitmap_portable_deserialize_js(roaring_bitmap_t * bitmap, const char * buf, uint32_t size) {
  ...
  size_t bytesread;
  if (!ra_portable_deserialize(&bitmap->high_low_container, buf, size, &bytesread)) return 202;

  return 0;
}

int roaring_bitmap_portable_serialize_js(roaring_bitmap_js_t * b) {
  if (b == NULL) return 301;

  size_t size = roaring_bitmap_portable_size_in_bytes(&b->b);
  char * ptr = (char *)malloc(size);
  if (ptr == NULL) return 302;
...
int roaring_bitmap_native_serialize_js(roaring_bitmap_js_t * b) {
  ...
  if (nativesize < portablesize && nativesize < MAX_SERIALIZATION_NATIVE_MEMORY) {
    ...
    serialized[0] = CROARING_SERIALIZATION_ARRAY_UINT32;  // Marker
    ...
    serialized[0] = CROARING_SERIALIZATION_CONTAINER;  // Marker
...
int roaring_bitmap_native_deserialize_js(roaring_bitmap_t * bitmap, const char * buf, uint32_t size) {
  ...
  switch (buf[0]) {
    case CROARING_SERIALIZATION_ARRAY_UINT32: {
      if (size == 1) return 0;  // Empty.
      if (size < 5) return 102;  // Wrong size!

      uint32_t card;
      memcpy(&card, buf + 1, sizeof(uint32_t));
      if (card == 0) return 0;  // Empty.
